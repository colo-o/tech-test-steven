...
const MovieCard = ({ movie, isSelected, handleOnClick }: MovieCardProps) => {
  const CardClassName = classNames('w-[180px] h-[100%] col-span-1 bg-cover rounded-2xl overflow-hidden relative',
  {
    'border-4 border-primary': isSelected
  })

  return <li className={CardClassName} style={{ backgroundImage: `url(${movie.photoUrL})` }}>
    <Button className="absolute top-2 right-4 text-xl" onClick={() => handleOnClick(movie)}>{isSelected ? '-' : '+'}</Button>
